setwd("/lustre/home/acct-clsyzs/clsyzs/SunJianle/singleCellMR/Roche/MR")
...
cum_mat <- map_dfr(c("../eQTL/pace_cum_ms_bc.csv","../eQTL/pace_cum_ctr_bc.csv"),fread)
setnames(cum_mat,"sub","id")
idss <- fread("../eQTL/id_slt.txt")
cum_mat <- cum_mat[id%in%idss$ID]
# snps <- fread("../genotype_chr/imputed/merged.raw")
# colnames(snps)[-seq(1,6)] <- sapply(colnames(snps)[-seq(1,6)], function(x) strsplit(x, "_")[[1]][1])

eqtl <- fread("../eQTL/eqtl_cis.csv")%>%filter(pvalue<0.001)

eqtl2 <- fread("../eQTL/eqtl_trans.csv")%>%filter(FDR<0.01)
# 
eqtl <- rbind(eqtl,eqtl2)


snps_all <- fread("../eQTL/SNP.txt")
snps_all <- snps_all[id%in%unique(eqtl$snps)]
...
genelist <- genes
outpath <- "results_oligo/pace_all_new4"
  ...
  tryCatch(
    {
      ...
      snps <- snps_all[id %in% IV,][order(match(id,IV))][,-1]
  ...
  tryCatch(
    {
      ...
      df <- cum_mat %>%
        left_join(snps %>% dplyr::select(id,any_of(IV)),by="id") %>%
        dplyr::rename(xcum=any_of(geneName),outcome="outcome")
...
res_mrl <- foreach(geneName = genelist,.packages = loaded_packages) %dopar% {
  MR_pace_lasso(geneName, m = timeLength, repeats = 2000)
}
...
res_mrl <- foreach(geneName = genelist,.packages = loaded_packages) %dopar% {
  MR_pace_lasso_linear(geneName, repeats = 2000)
}
res_mrl <- purrr::reduce(res_mrl,rbind)
colnames(res_mrl) <- c("IVnum","F","beta","ci_l","ci_u","se","p")
res_mrl <- as_tibble(res_mrl)
